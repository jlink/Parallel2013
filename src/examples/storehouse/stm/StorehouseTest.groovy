...
import examples.storehouse.immutable.Product
...
class StorehouseTest {
	...
	public void functionality() throws Exception {
		def store = new examples.storehouse.stm.Storehouse()
		assert store.newShelf('A', 5) instanceof examples.storehouse.immutable.Shelf
		assert store['A'] instanceof examples.storehouse.immutable.Shelf
	...
	public void concurrentMove() throws Exception {
		(1..100).each { n->
			final threads = []
			//println "${n}th trial"
			final concStorehouse = new examples.storehouse.stm.Storehouse()
